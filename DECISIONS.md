...
## 2. Trade-offs we weighed

**Device `TS` for windowing vs. dedup only.** We initially considered using the
device-reported timestamp to drive event-time windowing, since that's the more
"correct" event-time semantically. We backed away from this once we assumed device
clocks may not be reliable — a skewed device clock could distort watermark
progression for the whole keyed stream. We chose to trust `apiIngestTs` (assigned
once, at a single point we control) for ordering/windowing, and demoted device `TS`
...
pipelines don't scale operationally past a few thousand tenants. We chose a
...
pipeline. We explicitly accepted that this only solves *cross-tier* isolation,
...
**Exactly-once checkpointing vs. simplicity.** We enabled exactly-once
...
- **No tier routing or multiple ETL jobs in the runnable code.** We designed the
...
1. **Add support for more device types** - Currently the ETL only supports a temperature
   device readings. The ETL needs to be able to accept multiple reading parameters
   (up to a pre-determined payload size)
...
6. **Add the error-handling and metrics/alerting design** we explicitly
